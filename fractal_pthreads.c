...
void* iterateThroughEachPixel(void* arg) { 
    ...
    int thread_index = ID;
    ID++;
    int start_index = thread_index * (WIDTH / NUMBER_OF_THREADS);
    int stop_index = (thread_index + 1) * (WIDTH / NUMBER_OF_THREADS);
    pthread_mutex_unlock(&lock);
    int pixelX, pixelY;
    uint32_t color;
    
    for(pixelX = start_index; pixelX < stop_index; pixelX++) {
		for(pixelY = 0; pixelY < HEIGHT; pixelY++){
			color = get_color(pixelX, pixelY, WIDTH, HEIGHT, args->rotate);
...
void render_image(char * filename, double rotate){
    ...
    for (int i = 0; i < NUMBER_OF_THREADS; i++) {
		...
		args.rotate = rotate;
        pthread_create(&threads[i], NULL, iterateThroughEachPixel, (void*)&args); 
    }
